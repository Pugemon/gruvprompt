...
gruv_user_info(){
  ...
  if [[ ${GRUV_SECTION_USER_HOST} == 1 ]]; then
    __user+="@%M"
  fi
  echo -n "${__user}"
}
...
gruv_time_info(){
  [[ -z ${GRUV_SECTION_TIME_FORMAT} ]] && echo -n "$(date +%r)" || echo -n "$(date +"${GRUV_SECTION_TIME_FORMAT}")"
}
...
gruv_battery_stat(){
  ...
  if [[ ${__os} = "Linux" ]]; then
    if command -v upower > /dev/null 2>&1 ; then
      __bat_power=$(upower -i /org/freedesktop/UPower/devices/battery_BAT1 | grep state | awk '{print $2}')
      __bat_power_ind=""
      ...
      __bat_per=$(upower -i /org/freedesktop/UPower/devices/battery_BAT1 | grep percentage | awk '{print $2}' | sed "s|%||g")
      if [[ -n ${__bat_per} ]]; then
