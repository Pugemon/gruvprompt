...
gruv_prompt_render_left() {
  [[ ${#GRUV_SECTIONS_LEFT} -ne 0 ]] || return
  local __render_mode=$1
  local __separator=$GRUV_SECTIONS_LEFT_SEP_SYM
  local __last_bg=
  local __last_sep=
  for section in $GRUV_SECTIONS_LEFT; do
    ...
    local __section_function="gruv_prompt_section_${__section_name}"
    ...
    then
      ...
      [[ -n "$__last_sep" ]] && gruv_prompt_colorize "$__last_sep" "$__last_bg" "$__bg"
      __last_bg="$__bg"
      [[ -n $__sep ]] && __last_sep="$__separator" || __last_sep=""
      gruv_prompt_colorize "$__content" "$__fg" "$__bg"
    fi
  done
  [[ -n "$__last_sep" ]] && gruv_prompt_colorize "$__last_sep" "$__last_bg"
}

gruv_prompt_render_right() {
  [[ ${#GRUV_SECTIONS_RIGHT} -ne 0 ]] || return
  local __render_mode=$1
  local __separator=$GRUV_SECTIONS_RIGHT_SEP_SYM
  local __last_bg=
  ...
  for section in $GRUV_SECTIONS_RIGHT; do
    ...
    then
      ...
      local __bg=${__section[background]}
      local __sep=${__section[separator]+${__section[separator]}}
      [[ -n $__sep ]] && gruv_prompt_colorize "$__separator" "$__bg" $__last_bg
      __last_bg="$__bg"
      gruv_prompt_colorize "$__content" "$__fg" "$__bg"
    fi
  done
